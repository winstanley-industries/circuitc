...
const SOURCE_REVISION: &str = env!("CIRCUITC_APGAR_SOURCE_REVISION");
const MAX_EXECUTABLE_BYTES: u64 = 2 * 1024 * 1024 * 1024;

fn main() {
    let arguments: Vec<_> = env::args_os().collect();
    if arguments.len() != 3 {
        eprintln!("usage: apgar_route_provenance_writer EXECUTABLE OUTPUT");
        std::process::exit(2);
    }
    ...
    let provenance = format!(
        "circuitc-apgar-route-provenance-v1\nname=circuitc-apgar-route\nversion=1\ncontract=apgar-board-ir-v1+geometry-compiler-v1+candidate-policy-v1+route-candidate-v1.0\nsource_revision={SOURCE_REVISION}\nexecutable_sha256={digest}\ndevice_class=cpu-reference-v1\n"
    );
