...
| `CC-REQ-KICAD-007` | On a supported host, a clean checkout passes the explicit KiCad 10 gate (`//:kicad10_drc_test`) for parsing, ERC, DRC, connectivity, and schematic parity without relying on manual editor state. |
...
M1B required end-to-end proof, but its implementation combined several
independently reviewable contracts: source and Design IR, the vendored catalog
and KiCad lowering, report normalization, transactional publication, and host
acceptance. Future epics should keep one integrated acceptance plan while
landing those contracts as dependency-ordered PRs. That preserves vertical
integration while reducing the full-diff surface that every review revision
must re-audit.
...
| `CC-REQ-KICAD-003` | Bazel `compile_data`, `libraries/CircuitC.kicad_sym`, `libraries/CircuitC.pretty/`, and generated `${KIPRJMOD}` library tables | Static catalog definitions expose footprint geometry, drawing geometry, typed table metadata, and publishable files without constructing discarded owned footprints; `CompiledArtifacts` carries the sorted library files selected by the design. Catalog completeness tests derive their symbol and footprint sets from the part-definition table, and injected unavailable-file/geometry cases pin `CC-KICAD-FOOTPRINT-005/006/007`. Consistency tests tie each symbol pin number to its exact catalog offset, cross-check every silkscreen and courtyard field against the vendored footprint, pin the complete populated table structure and URI, and pin emitted-board drawing geometry together with each graphic's identity UUID; the host gate exports the symbol and footprint with isolated configuration and no user-global dependency. |
| `CC-REQ-KICAD-004` | `src/kicad.rs`, `CompiledArtifacts`, `//cmd/circuitc`, and the Rust fixture generator | Full-bundle CLI tests compare source builds across filenames, including identity maps, and against the Rust oracle; the required Rust gate independently repeat-compiles the physical-only/no-connect fixture and compares its artifacts and identity map. The physical-only fixture uses one ideal voltage source and a resistive load across TEST/GND; default-suite assertions pin the resulting nonsingular SPICE netlist. The default test also validates the emitted project JSON and both library tables, including design-derived `.kicad_pro` metadata for a second design name. Vendored file and table paths cross the public artifact boundary as canonical portable relative-path values that reject aliasing spellings and host-dependent separators. Component-specific tests pin exact source values and part-identity property values in both schematic and PCB artifacts, including a physical-only part, a mutated value, and the absence of manufacturer fields on a virtual part. Schematic tests pin catalog symbol-pin numbers independently from logical pins, project/root/reference instance stanzas for physical and virtual symbols, root sheet-instance metadata, and `in_bom`/`on_board` for both physical and virtual symbols, while board tests prove the footprint `sheetfile` follows two distinct design names. Unit and process-boundary tests pin unsafe generated-path rejection before root creation, symlinked-ancestor and target rejection, write-only regular-target replacement through directory authority, the documented error for fully inaccessible targets, transactional containment, exit status, actionable diagnostics, a racing destination that the real no-replace syscall preserves without staging residue, checked staging-file synchronization whose injected failure preserves originals and removes temporary files, and post-publication synchronization of distinct artifact and created-directory parents whose injected failure remains visible without misreporting completed publication. The host gate compares independent source builds byte for byte. |
| `CC-REQ-KICAD-005` | Exact schematic/board placement and explicit route lowering in the language, Design IR, and KiCad backend | Full-turn and declaration-order tests preserve canonical IR and artifacts; route UUIDs remain stable when geometry moves, and all coordinates lower from integer nanometres without floating point. Canonical anchor and transformed pin-point collision tests reject schematic placements that could merge distinct connection states or merge distinct no-connect pins, and anchor-collision diagnostics retain the first component as a related semantic path. The schema and language fix counterclockwise rotation in KiCad's Y-down frame; asymmetric unit tests pin every orthogonal board transform, back-layer tests pin pad, silkscreen, and courtyard layers, and a KiCad-host fixture cross-checks schematic rotation with a physical-only resistor whose pins have different connected and no-connect states. |
| `CC-REQ-KICAD-006` | Deterministic `KicadIdentity` enumeration, `<design>.kicad-map.json`, [`schemas/kicad_identity_map/v1.md`](../../schemas/kicad_identity_map/v1.md), and `tools/kicad/normalize_drc.py` | Frontend collision provenance and backend UUID emission consume one generated-identity descriptor enumeration with explicit authored-origin categories. Tests assert exact set equality between every schematic/board UUID and the identity map for both reference fixtures, plus UUID and semantic-path uniqueness. Global rendered-semantic-path collision tests include source primary/related spans for structural, footprint, and footprint-graphic identities. A provenance-only collision between an authored component and a generated placement path compiles without an identity collision but deliberately emits `location: null`; the normalizer fixture proves that nullable location survives into a mapped finding's CircuitC block. Derived connection diagnostics resolve to primary and related authored component spans; an exact route identity retains the route's source location while a route whose path is only a generated-identity prefix cannot capture derived component provenance. The full-bundle CLI test feeds its freshly generated map into the strict ERC normalizer. Schema/type/exact-field/source/UUID/path/missing/duplicate/location manifest tests, ERC major-version and sheet object/path/violations/coordinate-shape rejection, shared finding object/items validation, ignored-check description validation, explicit DRC/ERC report-kind assertions, missing and unknown finding-UUID rejection, exact severity and ignored-check policy, and multi-finding source-correlated ERC/DRC/unconnected/parity aggregation pass under Bazel. |
| `CC-REQ-KICAD-007` | `//:kicad10_drc_test`, `//:kicad_project_validator_test` | KiCad 10.0.5 parsed/exported the vendored libraries and ran isolated ERC/DRC/parity for both the voltage divider and the source fixture containing a rotated physical-only/no-connect component. Each reported 0 ERC violations, 0 DRC violations, 0 unconnected items, and 0 parity issues. This host result also authorizes the board emitter's deliberate name-only `(net "NAME")` pad/segment references. The deterministic project validator accepts both generated project files and rejects malformed JSON, non-object roots, missing or extra top-level fields, invalid or nonempty nested structure, and filename mismatch. |
...
`local` and `manual`; the recorded host evidence was produced on a local macOS
host with KiCad 10.0.5 and is not reproduced by Linux CI. This is an explicit
reviewer decision: the tags remain intentional because the supported KiCad
executable is platform-local, and completion claims reproducibility through
the named supported-host gate rather than execution by the default Linux
suite. The following SHA-256 values are only the historical snapshot bound to
`b616d52e069678c66f40e1b0487e282d36d2a79e` above:
