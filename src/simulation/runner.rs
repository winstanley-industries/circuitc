...
fn configure_unix_process(command: &mut Command, wall_limit: Duration, limits: &OhmnivoreLimits) {
    ...
    #[cfg(test)]
    let core_bytes = limits.core_bytes;
    #[cfg(not(test))]
    let core_bytes = 0;
    ...
    unsafe {
        command.pre_exec(move || {
            ...
            set_limit(libc::RLIMIT_CORE, core_bytes)?;
            Ok(())
        });
    }
}
...
fn normalize_csv(
    ...
    verify_result_inventory(&result, map, deadline)?;
    Ok(result)
}

fn verify_result_inventory(
...
mod tests {
    ...
    #[test]
    fn result_inventory_mapping_honors_the_aggregate_deadline() {
        let fixture = fixture(AnalysisKind::DcOperatingPoint, ".OP", AxisKind::Scalar, 0.0);
        ...
        let error =
            verify_result_inventory(&result, &map, Instant::now() - Duration::from_millis(1))
                .unwrap_err();
    ...
    fn digest_rebound_identity_mutants_fail_before_process_execution() {
        ...
        let mut unsupported_map = map.clone();
        unsupported_map
            .devices
            .iter_mut()
            .find(|device| device.semantic_path == "runner.r_top")
            .unwrap()
            .backend = "L1".to_owned();
        let unsupported_device = rebind_fixture(
            &fixture,
            fixture
                .netlist
                .replacen(
                    "* @circuitc-device 72756E6E65722E725F746F70 5231 R1",
                    "* @circuitc-device 72756E6E65722E725F746F70 5231 L1",
                    1,
                )
                .replacen("R1 VIN VOUT 10e3", "L1 VIN VOUT 1e-3", 1),
            unsupported_map,
        );
        let extra_device_field = rebind_fixture(
            &fixture,
            fixture
                .netlist
                .replacen("R1 VIN VOUT 10e3", "R1 VIN VOUT 10e3 TEMP=27", 1),
            map.clone(),
        );
        let noncanonical_value = rebind_fixture(
            &fixture,
            fixture
                .netlist
                .replacen("R1 VIN VOUT 10e3", "R1 VIN VOUT 10k", 1),
            map.clone(),
        );
        let missing_dc_keyword = rebind_fixture(
            &fixture,
            fixture
                .netlist
                .replacen("V1 VIN 0 DC 10", "V1 VIN 0 dc 10", 1),
            map,
        );

        let body = ": > \"${0%/*}/invoked\"; exit 99";
        ...
        for mutant in [
            ...
            assert!(!root.join("invoked").exists(), "fake backend was invoked");
        }
    ...
    fn process_capture_enforces_environment_timeout_and_output_caps() {
        assert!(
            ...
            "the parent test process must expose a non-allowlisted variable"
        );
        ...
        let isolated = run_process(
            ...
            [
                OsStr::new("-c"),
                OsStr::new(
                    "test -z \"${TEST_TMPDIR+x}\" && test \"$LC_ALL\" = C && test \"$RAYON_NUM_THREADS\" = 1 && test -d \"$HOME\" && test -d \"$TMPDIR\" && printf ok",
                ),
            ],
    ...
    #[cfg(unix)]
    #[test]
    fn unix_process_limits_are_attached_and_file_size_is_enforced() {
        let root = test_root("process-limits");
        let directory = ScopedWorkDirectory::create(&root).unwrap();
        let mut limits = OhmnivoreLimits::default();
        limits.analysis_wall = Duration::from_secs(2);
        limits.file_bytes = 4_096;
        limits.core_bytes = 4_096;
        ...
        let probe = run_process(
            ...
            [
                OsStr::new("--exact"),
                OsStr::new("simulation::runner::tests::unix_process_limit_probe"),
                OsStr::new("--nocapture"),
            ],
            limits.analysis_wall,
            &limits,
        ...
        let stdout = std::str::from_utf8(&probe.stdout).unwrap();
        let expected = format!(
            "CIRCUITC_LIMIT_PROBE cpu=3/3 fsize=4096/4096 nofile={0}/{0} core=4096/4096",
            limits.open_files,
        );
        ...
        let oversized = run_process(
            ...
            [
                OsStr::new("-c"),
                OsStr::new("set -e; /bin/dd if=/dev/zero of=too-large bs=8192 count=1 2>/dev/null"),
            ],
            limits.analysis_wall,
            &limits,
        )
        ...
        assert!(
            ...
            "file-size limit did not stop the oversized write"
        );
        if let Ok(metadata) = fs::metadata(directory.path().join("too-large")) {
            assert!(metadata.len() <= limits.file_bytes);
        }
    ...
    #[cfg(unix)]
    #[test]
    fn unix_process_limit_probe() {
        let cpu = probe_limit(libc::RLIMIT_CPU);
        let file = probe_limit(libc::RLIMIT_FSIZE);
        let open = probe_limit(libc::RLIMIT_NOFILE);
        let core = probe_limit(libc::RLIMIT_CORE);
        print!(
            "CIRCUITC_LIMIT_PROBE cpu={}/{} fsize={}/{} nofile={}/{} core={}/{}",
            cpu.0, cpu.1, file.0, file.1, open.0, open.1, core.0, core.1,
        );
        #[cfg(target_os = "linux")]
        {
            let address_space = probe_limit(libc::RLIMIT_AS);
            print!(" address-space={}/{}", address_space.0, address_space.1);
        }
        println!();
    }

    #[cfg(unix)]
    fn probe_limit(resource: RlimitResource) -> (libc::rlim_t, libc::rlim_t) {
        let mut limit = std::mem::MaybeUninit::<libc::rlimit>::uninit();
        // SAFETY: `limit` points to writable storage for one `rlimit`, and the
        // resource values are the platform constants used by production.
    ...
    fn full_runner_enforces_cpu_argv_environment_timeout_and_fresh_cleanup() {
        ...
        let result = runner
            .execute_with_pre_netlist_hook(
                fixture.netlist.as_bytes(),
                fixture.request.as_bytes(),
                fixture.map.as_bytes(),
                |netlist_path| std::os::unix::fs::symlink("/dev/null", netlist_path),
            )
            .unwrap();
        assert_eq!(result.status, ExecutionStatus::Failed);
        ...
        assert!(
            remaining.is_empty(),
            "symlink-rejected work directory was not cleaned"
        );
        fs::remove_dir_all(root).unwrap();
